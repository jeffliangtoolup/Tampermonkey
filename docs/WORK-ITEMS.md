...
| 3 | **2026-08-20** (*revised 2026-08-20: both homes are tracked but never pushed — see #8*) — The NetSuite side of this integration lives in **this repo's own** `netsuite-sb/` and `netsuite-prod/`; the workspace-level homes of those names are never written to. Both homes were created with a `README.md` and nothing else. *Rejected:* seeding a first feature pair immediately — no feature is scoped yet (see W2), and a pre-made slice named for nothing is what stops slices being per-feature. |
...
| 7 | **2026-08-20** — The NetSuite MCP **never writes to production from this repo**, and writes to sandbox **only with explicit per-call approval**; reads against either account are unrestricted. Enforced by `scripts/netsuite-mcp-guard.py` as a `PreToolUse` hook, not by rule alone — a written rule does not bind a session running in an auto-approving permission mode. The guard fails closed: an unknown tool, unknown method, or unreadable payload counts as a prod write. *Accepted gap:* a RESTlet invoked with `GET` classifies as a read, since HTTP method is the only signal available without knowing what the script does. *Rejected:* expressing the rule as permission `deny` entries — those cannot match arguments, so they would have blocked prod reads along with prod writes. The rule holds until deliberately deleted; `CLAUDE.md` § HARD SAFETY RULES owns the removal procedure. |
| 8 | **2026-08-20** — `origin` is a **public** repo not owned by this project's maintainer, so: (a) credentials, internal identifiers (account numbers, `_SB<n>` realms), and proprietary data (customer names, order and margin figures, live record dumps) never enter a tracked file — `scripts/check-secrets.sh` gained shape-based detectors for the first two classes and refuses the commit; (b) `netsuite-sb/` and `netsuite-prod/` are **tracked but never pushed**, enforced by a new `.githooks/pre-push`; (c) the account numbers already committed earlier today were scrubbed from `docs/ARCHITECTURE.md` and both slice READMEs. *Accepted limits:* the scrub does not remove them from the pushed history (that needs a force-push to a remote shared with the repo owner, judged not worth it for a non-credential), and proprietary data has no detector — that class rests on judgment. *Rejected:* literal account numbers in the detector list, which would put the number in a tracked file and move the leak rather than close it. *Workflow consequence:* git cannot filter a push by path, so a commit touching either slice home makes its branch unpushable while it is in the range — NetSuite work belongs on a branch that is never pushed. *Recorded exception, 2026-08-20:* the scrub in (c) was pushed with `git push --no-verify` on Jordan's explicit instruction, because the never-push rule otherwise made it impossible to remove those account numbers from the remote's current tree — the rule forbids pushing the very paths the fix has to touch. The diff removed two lines of prose and added nothing, which is why the exception was judged safe. It is a one-off: the rule holds for every subsequent push, and a future exception needs the same explicit instruction. |
